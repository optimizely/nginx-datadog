...
use crate::{configuration, error::Error, snippet::generate_snippet};
...
#[no_mangle]
pub extern "C" fn snippet_create_from_json(json: *const c_char) -> *mut Snippet {
    let json_bytes = unsafe { CStr::from_ptr(json) }.to_bytes();

    let result = serde_json::from_slice::<configuration::Configuration>(json_bytes)
        .map_err(Error::Json)
...
#[no_mangle]
#[deprecated(note = "use snippet_create_from_json")]
pub extern "C" fn snippet_create_from_struct(configuration: Configuration) -> *mut Snippet {
    let Some(configuration) = (unsafe { configuration.validate() }) else {
        return null_mut();
    };

    let Ok(snippet_content) = generate_snippet(&configuration) else {
        return null_mut();
    };

    SnippetInternal::new(Ok(snippet_content)).into_public_ptr()
}
...
#[no_mangle]
pub extern "C" fn snippet_cleanup(snippet: *mut Snippet) {
    _ = unsafe { SnippetInternal::from_public_ptr(snippet) };
}

/// Get the snippet bytes length
#[no_mangle]
#[deprecated(note = "use snippet->length")]
pub extern "C" fn snippet_get_length(snippet: *const Snippet) -> u32 {
    unsafe { SnippetInternal::from_public_ptr(snippet) }.length
}
...
pub struct RumConfiguration {
    ...
    site: *const c_char,
    default_privacy_level: *const c_char,
    track_resources: bool,
    session_sample_rate: f32,
}

impl RumConfiguration {
    unsafe fn validate(&self) -> Option<Value> {
        Some(json!({
            "clientToken": cstr_to_json_str(self.client_token)?,
            "applicationId": cstr_to_json_str(self.application_id)?,
            "site": cstr_to_json_str(self.site)?,
            "defaultPrivacyLevel": cstr_to_json_str(self.default_privacy_level)?,
            "trackResources": self.track_resources,
            "sessionSampleRate": self.session_sample_rate,
        }))
    }
}

impl Configuration {
    unsafe fn validate(&self) -> Option<configuration::Configuration> {
        if self.major_version != 5 {
            return None;
        }
        Some(configuration::Configuration {
            major_version: self.major_version,
            rum: self.rum.validate()?,
        })
    }
}

unsafe fn cstr_to_json_str(cstr: *const c_char) -> Option<Value> {
    if cstr.is_null() {
        Some(Value::Null)
    } else {
        Some(Value::String(CStr::from_ptr(cstr).to_str().ok()?.into()))
    }
}
...
mod tests {
    ...
    #[test]
    fn snippet_creation() {
        let snippet = snippet_create_from_json(cstr_ptr(b"{\"majorVersion\":5,\"rum\":{}}\0"));
    ...
    fn snippet_creation_errors() {
        // Invalid UTF-8
        assert_eq!(
            get_error(snippet_create_from_json(cstr_ptr(&[0xAA, 0]))),
            (
                1,
        ...
        // Invalid JSON
        assert_eq!(
            get_error(snippet_create_from_json(cstr_ptr(b"{\0"))),
            (
                1,
        ...
        assert_eq!(
            get_error(snippet_create_from_json(cstr_ptr(
                b"{\"majorVersion\":1,\"rum\":{}}\0"
            ))),
            (2, "Unsupported major version 1".to_owned())
        );
