...
use crate::{injector, Snippet, SnippetInternal};
...
/// ```
#[no_mangle]
pub extern "C" fn injector_create(snippet: *const Snippet) -> *mut Injector {
    let snippet_content = match unsafe { SnippetInternal::borrow_public_ptr(snippet) }.inner {
        Ok(ref content) => content.as_slice(),
        Err(_) => &[],
    };

    Box::into_raw(Box::new(Injector {
...
#[no_mangle]
pub extern "C" fn injector_cleanup(injector: *mut Injector) {
    _ = unsafe { Box::from_raw(injector) };
}
...
/// ```
#[no_mangle]
pub extern "C" fn injector_write(
    injector: *mut Injector,
    chunk: *const u8,
    length: u32,
) -> InjectorResult {
    let ffi_injector = unsafe { &mut *injector };
    let chunk = unsafe { slice::from_raw_parts(chunk, length as usize) };
    InjectorResult::new(
...
/// }
#[no_mangle]
pub extern "C" fn injector_end(ffi_injector: *mut Injector) -> InjectorResult {
    let ffi_injector = unsafe { &mut *ffi_injector };
    InjectorResult::new(
